...
namespace Exoa.Designer
{
    public class FloorMapSerializer : BaseSerializable, IDataSerializer
    {
        ...
        {
            //Debug.Log("OnRequestFloorActionHandler action:" + action);
        ...
        private void OnFileSaved(string fileName, FileType fileType)
        {
            ...
            {
                perspViewName = HDSettings.FLOORMAP_DESIGNER_THUMBNAIL_PREFIX + floorsMenu.CurrentFloorId + "_persp";
            ...
            //string topViewName = fileName.Replace(".json","")+ "_top";
            perspViewName = fileName.Replace(".json", "") + "_persp";
            ...
            if (roomsGo != null)
                ThumbnailGeneratorUtils.TakeAndSaveScreenshot(roomsGo.transform,
                    HDSettings.FLOORMAP_DESIGNER_THUMBNAIL_PREFIX + perspViewName, false, new Vector3(1, -1, 1));
        ...
        public void DuplicateFloor(string uniqueFloorId)
        {
            ...
            ThumbnailGeneratorUtils.Duplicate(
                HDSettings.FLOORMAP_DESIGNER_THUMBNAIL_PREFIX + uniqueFloorId + "_persp",
                HDSettings.FLOORMAP_DESIGNER_THUMBNAIL_PREFIX + floor.uniqueId + "_persp");
        ...
        public void Clear(bool clearFloorsUI = true, bool clearFloorMapUI = true, bool clearScene = true)
        {
            ...
            if (clearFloorsUI)
            {
                currentFloorMapFile = GetNewEmptyData();
            }
        }
        ...
        {
            currentFloorMapFile = GetNewEmptyData();
            string str = JsonConvert.SerializeObject(currentFloorMapFile, Formatting.Indented, new JsonSerializerSettings()
            {
                ReferenceLoopHandling = ReferenceLoopHandling.Ignore
            });
            return str;
