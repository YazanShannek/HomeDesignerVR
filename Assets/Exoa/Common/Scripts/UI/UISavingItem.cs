...
namespace Exoa.Designer
{
    public class UISavingItem : MonoBehaviour
    {
        ...
        public void LoadImages(UISaving.ThumbnailPrefixes prefix)
        {
            string perspViewName = prefix + "_" + fileName + "_persp";
            string buildingViewName = HDSettings.BUILDING_THUMBNAIL_PREFIX + fileName + "_persp";

            //Texture2D t1 = ThumbnailGeneratorUtils.Load(thumbnailPrefix + "_" + topViewName);
            ThumbnailGeneratorUtils.Exists(buildingViewName, (bool exists) =>
            {
                if (exists)
                {
                    ThumbnailGeneratorUtils.Load(buildingViewName, (Texture2D t) =>
                    {
                        SetImages(null, t);
                    },
                    (UnityWebRequest req, string msg) =>
                    {
                        SetImages(null, Resources.Load<Texture2D>("Thumbnails/EmptyThumb"));
                    });
                }
                else
                {
                    ThumbnailGeneratorUtils.Load(perspViewName, (Texture2D t) =>
                    {
                        SetImages(null, t);
                    },
                    (UnityWebRequest req, string msg) =>
